...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        // 总个数显示文本
        var total_num_str = settings.data.length + " Item";

        // 分组总个数
        var total_group_num = getGroupNum(groupData, groupListName);
        // 分组总个数显示文本
        var total_group_num_str = total_group_num + " Item";
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                if ($(transferId).find(tabContentFirst).css("display") != "none") {
                    ...
                    $(transferId).find(groupTotalNum).empty();
                    $(transferId).find(groupTotalNum).append($(transferId).find(transferDoubleGroupListLiUlLi).length + " Item");
                } else {
                    // 分组
                    ...
                    $(transferId).find(totalNum).empty();
                    $(transferId).find(totalNum).append($(transferId).find(transferDoubleListLi).length + " Item");
                }
                // 数据变化触发回调
        ...
        $(addSelected).on("click", function () {
            ...
            if ($(transferId).find(tabContentFirst).css("display") != "none") {
                ...
                var new_total_num_str = new_group_total_num + " Item";
                // 左侧数量
                $(transferId).find(groupTotalNum).append(new_total_num_str);
                // 右侧数量
                $(transferId).find(selectedTotalNum).text(selected_total_num + " Item");
                ...
                // 计算右侧总数
                selected_total_num = selectedItemNum;
                var new_total_num_str = new_total_num + " Item";
                // 左侧数量
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + '">Item</div>'
            + '</div>'
